...
Current version in HASS requires a working Ping, but that seems tricky to setup and I don't need it
Updated the "update" method to check if a Host is set and only then try the ping, it is useless otherwise
"""
import logging
import platform
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    add_devices([WOLSwitchFix(hass, name, host, mac_address, off_action)])


class WOLSwitchFix(SwitchDevice):
    ...
    def update(self):
        ...
        if self._host:  # Only usefull if host is configured
