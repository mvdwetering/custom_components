...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    for port_num, port_name in ports.items():
        binary_sensors.append(RPiGPIOPollBinarySensor(
            port_name, port_num, pull_mode, invert_logic))
    add_devices(binary_sensors, True)


class RPiGPIOPollBinarySensor(BinarySensorDevice):
    """Represent a binary sensor that uses Raspberry Pi GPIO."""

    def __init__(self, name, port, pull_mode, invert_logic):
    ...
    def should_poll(self):
        """Use polling, thats the entire idea."""
        return True
